...
class ContentsDAO {
    ...
    static inserir(conteudo) {
        const query = 'INSERT INTO conteudos (titulo, descricao, posicao) VALUES (?, ?, ?)';
        return new Promise((resolve, reject) => {
            db.run(query, [conteudo.titulo, conteudo.descricao, conteudo.posicao], function (err) {
                if (err) {
                    reject({
    ...
    static atualizar(id, conteudo) {
      const query = 'UPDATE conteudos SET titulo = ?, descricao = ?, posicao = ? WHERE id = ?';
      return new Promise((resolve, reject) => {
          db.run(query, [conteudo.titulo, conteudo.descricao, conteudo.posicao, id], (err) => {
              if (err) {
                  reject({
